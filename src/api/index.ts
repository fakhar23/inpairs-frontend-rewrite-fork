...
export async function getUserList(queryString: string) {
  const result = await axiosInstance.get(ENDPOINTS.users + queryString);
  return result.data;
}
export async function updateUserStatus({ id, ...payload }: UpdateUserStatus) {
