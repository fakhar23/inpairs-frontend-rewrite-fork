...
function UserProfileLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <>
      <nav className="relative bg-red200 bg-gradient-to-r from-red500 to-navDarkPurple px-[4rem] pt-[2rem] pb-[3rem] text-white mb-2">
              ...
              <>
                {transitions((style, item) =>
                  item ? (
                    <animated.ul
                      style={style}
                      className={`overflow-hidden border-orange block border nav absolute top-10 right-0 z-10 w-56 origin-top-right rounded-md bg-white shadow-lg text-darkBlack ease-in-out duration-300 [&_*]:transition-all [&_*]:duration-300 `}
                    >
                      <Link
                        onClick={() => setOpenNav(false)}
                        href={"/profile/me"}
                      >
                        <li className="hover:bg-primaryColor text-gray block px-4 py-2 text-sm hover:bg-orange hover:bg-opacity-50">
                          Profile
                        </li>
                      </Link>
                      <Link
                      ...
                      <li
                        className="hover:bg-primaryColor hover:bg-opacity-50 text-gray block px-4 py-2 text-sm  hover:bg-orange  cursor-pointer"
                        onClick={() => {
                          setOpenNav(false);
                          handleLogout();
                        }}
